...
class RejectReason(str, Enum):
    ...
    # Per-window prompt range (#91): prompt_idx fell outside the window's
    # eligible [lo, hi) slice. Only emitted once the validator arms
    # PROMPT_RANGE_ENFORCE_FROM_WINDOW; non-retryable (drop and pick again).
    ...
    # Wire-v2 (agent/wire-v2-cutover): client must advertise the validator's
    # exact wire version once the cutover is enforced. Added ahead of the
    # merge so a v2 validator response always parses.
    ...
    # v1-merkle-shadow (#119) / wire-v2: validator recomputes the canonical
    # rollout Merkle root (shadow-only on wire v1, enforced from v2).
    MERKLE_ROOT_MISMATCH = "merkle_root_mismatch"
    WRONG_RANDOMNESS = "wrong_randomness"
    # v1-admission-hardening (#114): only hotkeys registered on the subnet are
    ...
    # Upload-precommit handshake (upstream 8835a95). Returned at HTTP 200, so a
    # strict enum missing these raises ValidationError instead of yielding a
    # clean reject — that is how the v6 env_name break stayed hidden.
...
class RolloutSubmission(BaseModel):
    ...
    commit: dict[str, Any]
    env_name: str  # environment that generated this rollout; validator routes by this (v6)
...
class BatchSubmissionRequest(BaseModel):
    ...
    # Forced-seed protocol version (validator b790e42). 0 = pre-forced-seed
    ...
    def _rollouts_share_one_environment(self):
        # Wire-v2: the canonical Merkle root binds env_name per rollout and the
        # validator routes the whole group by one env. Fail fast locally.
...
class SubmissionPrecommitRequest(BaseModel):
    ...
    ``payload_sha256`` commits the exact serialized reveal and ``payload_bytes``
    binds its ingress accounting size. NOTE: ``randomness`` is signed into the
    binding but deliberately NOT transmitted — the validator substitutes its own
    window randomness, and ``extra="forbid"`` would reject the extra field.
    """
...
class Verdict(BaseModel):
    ...
    # Optional observability fields. The validator enriches recent verdicts;
    # older records omit them. Mirrors upstream so strict parsing of the live
    # validator's /verdicts payload never breaks, and exposes the `rewarded`
    # signal the MixController consumes.
...
class RolloutMetadata(BaseModel):
    ...
    # BFT (v7): forced=True when force-terminated at the thinking budget;
    # force_span = [start, end] token indices of the injected FORCE template
    # (validated byte-exact by the validator carve-out). truncated is
    # validator-set (cap-truncated / non-terminating), feeds reward shaping.
    forced: bool = False
