...
# Wire-v2 (agent/wire-v2-cutover): the envelope additionally binds the wire
...
def build_envelope_binding(
    *,
    ...
    if protocol_version is None:
        ...
        domain = ENVELOPE_DOMAIN_V2
        protocol_version_b = int(protocol_version).to_bytes(
            8, "big", signed=False,
        )
        parts = (hotkey_b, window_b, prompt_b, merkle_b, ckpt_b, round_b,
                 protocol_version_b, rand_b, nonce_b)
...
def build_precommit_binding(
    *,
    ...
    Byte-exact port of upstream ``8835a95``. This repeats the envelope's
    routing identity and additionally binds the environment, exact serialized
    byte count and digest, and protocol version, so the miner cannot replace a
    committed body with a same-sized reveal. Field ORDER is part of the wire
