...
Un seul fil de service : les forwards sont sérialisés sur le GPU, séquence par
séquence ``[1, seq_len]`` sans masque, exactement comme ``verifier.py``.
"""
...
def make_server(socket_path: str, state: ReplicaState):
    ...
    return socketserver.UnixStreamServer(socket_path, _Handler)
...
def main() -> None:
    ...
    logger.info("réplique prête sur %s", path)
