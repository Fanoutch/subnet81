...
while true; do
  ...
  if [ -z "$last" ]; then echo "$(date -u +%FT%TZ) WEDGE (aucun stream_fire dans le tail, proc ${page}s) — restart" >> "$WLOG"; tmux kill-session -t miner 2>/dev/null; pkill -9 -f "reliquary.cli.main mine"; pkill -9 -f EngineCore; sleep 8; tmux new-session -d -s miner "bash /workspace/start_miner.sh 2>&1 | tee -a $LOG"; sleep 600; continue; fi
