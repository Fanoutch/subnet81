...
# Mineur MATH : code dans son propre répertoire (le mineur code reste intact
# dans /workspace/reliquary-miner-priv, relançable tel quel).
RELIQUARY_MINER_DIR=${RELIQUARY_MINER_DIR:-/workspace/reliquary-miner-math}
export PYTHONPATH=$RELIQUARY_MINER_DIR
...
# ============================================================================
# MINEUR MATH (24/09) — openmathinstruct seul. Ce bloc ÉCRASE les réglages
# « code » posés plus haut ; le lanceur du mineur code (launch_miner_v4.sh)
# reste intact. Pourquoi math : prix d'émission par env (code 0,057 au
# plancher, math 0,529), lane math sous-alimentée depuis 46801 (mémoire
# project_prix_par_env_2409). Tout ce qui est appris sur les prompts CODE
# (table de scores, prior, modèles volume/risque, mémo, fantômes, index
# brûlés) est DÉBRANCHÉ : la table n'a pas de clé d'env (elle appliquerait les
# scores code aux index math) et le mémo ne filtre pas l'env.
# Vide = désactivé côté moteur (défauts du code). Sélection = tirage uniforme
# dans la tranche tant qu'aucun prior math n'existe.
# ============================================================================
MATH_DIR=${RELIQUARY_MATH_DIR:-/workspace/math}
mkdir -p "$MATH_DIR"
export RELIQUARY_ACTIVE_ENVS=openmathinstruct
export RELIQUARY_PROMPT_SCORES=""
# Prior MATH (24/09) : régression logistique positifs(R2)/non-étiquetés(OMI),
# scripts/train_prior_math_pu.py, C=0,02. Éval sur 620 de NOS étiquettes
# jamais vues : AUC 0,643, en zone top-10 % 56,5 % contre 33,2 % au hasard.
# Vide ⇒ tirage uniforme. 1 slot sur 8 reste au hasard (étiquettes non
# biaisées pour ré-entraîner/évaluer).
# 25/09 : prior math v2 = v1 − 0,6·indice de marge (mots + source OMI + réponse
# attendue), scripts/build_prior_math_v2.py. Éval 1 064 tirages au hasard :
# AUC 0,646 → 0,673, top-5 % 58,5 → 66 %. Juge en vol : en zone des tirages
# « ranked » contre le slot « explore ». REPLI : RELIQUARY_MATH_PREDICTOR=$MATH_DIR/predictor_math.json
export RELIQUARY_PROMPT_PREDICTOR=${RELIQUARY_MATH_PREDICTOR-$MATH_DIR/predictor_math_v2.json}
export RELIQUARY_EXPLORE_SLOTS=${RELIQUARY_MATH_EXPLORE_SLOTS:-1}
# Bake 8 → 16 (24/09 soir) : le bake suivant attend le dernier groupe (traîne
# math 30-60 s, GPU quasi à l'arrêt) alors que la lane se ferme vers 60 s ;
# 16 prompts = 2× plus de groupes précoces. Juge : payés math/fen R2 (réf ~5,3).
# REPLI : RELIQUARY_MATH_BAKE_BATCH_SIZE=8.
# 25/09 : FENÊTRE GLISSANTE — le bake reçoit 32 prompts classés, 16 en vol ;
# chaque groupe livré fait entrer le suivant. Mesuré nuit 24→25 : lane fermée
# à 45 s, bake suivant démarré à 67 s (attente de la traîne), 6,7 groupes
# envoyés avant la coupure sur ~10 en zone. Même charge GPU (16×16 séquences).
# REPLI : RELIQUARY_MATH_MAX_LIVE_PROMPTS=0 et RELIQUARY_MATH_BAKE_BATCH_SIZE=16.
export RELIQUARY_BAKE_BATCH_SIZE=${RELIQUARY_MATH_BAKE_BATCH_SIZE:-32}
# 25/09 après-midi : 16 → 20 en vol. MAX_NUM_SEQS reste 256 : les 64 requêtes
# en trop attendent dans la file vLLM et prennent la place de chaque ROLLOUT
# qui finit (remplissage au rollout près, mémoire inchangée). Juge : groupes
# prêts avant 40 s par bake. REPLI : RELIQUARY_MATH_MAX_LIVE_PROMPTS=16.
export RELIQUARY_STREAM_MAX_LIVE_PROMPTS=${RELIQUARY_MATH_MAX_LIVE_PROMPTS:-20}
export RELIQUARY_SHORT_RISK_MODEL=""
export RELIQUARY_VOLUME_MODEL=""
export RELIQUARY_BURNED_IDX=""
# Veto du cooldown de CONTENU par empreinte (math) : fichier poussé depuis la
# dev box par scripts/refresh_burned_digests.py --push (instantané R2).
export RELIQUARY_BURNED_DIGESTS="$MATH_DIR/burned_digests_math.txt"
export RELIQUARY_GHOST_BAKE=0
export RELIQUARY_GHOST_FEED=0
export RELIQUARY_GHOST_DUMP="$MATH_DIR/ghost_math.jsonl"
# Mémo : corpus math DÉDIÉ (le fichier code servirait des index code comme
# têtes math). Têtes mémo coupées tant que le corpus est vide ; à rallumer
# quand il contient des ex-payables math mesurés.
export RELIQUARY_SAMPLE_DUMP="$MATH_DIR/samples_math.jsonl"
export RELIQUARY_MEMO_SLOT=${RELIQUARY_MATH_MEMO_SLOT:-0}
export RELIQUARY_MEMO_HEAD_SLOTS=${RELIQUARY_MATH_MEMO_HEAD_SLOTS:-0}
export RELIQUARY_MEMO_RUN_START=0
export RELIQUARY_MEMO_MIN_VOL=0
export RELIQUARY_SZ_BLACKLIST_FILE="$MATH_DIR/sz_blacklist_math.json"
# Univers OMI complet (le biais « augmented_gsm8k » date de mai, autre modèle,
# et ne recoupe la tranche que sur les shards 0-3).
export RELIQUARY_OMI_SOURCES=""
# Miroir parquet : propre au code (lu seulement par opencodeinstruct.py).
unset RELIQUARY_PARQUET_LOCAL_ROOT RELIQUARY_PARQUET_EXPECTED_LEN
# Plafond de génération : 2500 (réglage code) jetterait localement tout
# groupe math dont un rollout dépasse 2500 sans EOS (rollout le plus long du
# groupe : p50 833, p90 2118 sur le marché). 24/09 14h : 4096 jetait 17 % des
# groupes (29/166, un rollout au-delà) → 8192 = plafond protocole (1 rollout
# tronqué toléré en math). Ancien texte : 4096 couvre la queue ; À
# MESURER en vol (taux de drop « bad termination » local) avant de bouger.
export RELIQUARY_MAX_NEW_TOKENS=${RELIQUARY_MATH_MAX_NEW_TOKENS:-8192}
# Lane math : budget d'admission 224/env, lane non remplie ~60 % des fenêtres.
export RELIQUARY_MAX_SUBMISSIONS_PER_WINDOW=${RELIQUARY_MATH_MAX_SUBMISSIONS:-128}
# Journaux séparés du mineur code (analyses et sauvegardes distinctes).
export RELIQUARY_VERDICTS_DUMP="$MATH_DIR/verdicts_math.jsonl"
export RELIQUARY_SUBMIT_DUMP="$MATH_DIR/submits_math.jsonl"
export RELIQUARY_FLIP_DUMP="$MATH_DIR/flip_math.jsonl"
export RELIQUARY_OOZ_DUMP="$MATH_DIR/ooz_math.jsonl"
export RELIQUARY_WINDOW_DUMP="$MATH_DIR/windows_math.jsonl"
[ -n "${RELIQUARY_PROOF_MARGIN_DUMP:-}" ] && export RELIQUARY_PROOF_MARGIN_DUMP="$MATH_DIR/proof_margin_math.jsonl"
[ -n "${RELIQUARY_DUMP_SUBMISSION:-}" ] && export RELIQUARY_DUMP_SUBMISSION="$MATH_DIR/submissions_dump_math.jsonl"
# 24/09 : le mineur math tourne sur sa propre H100 sous hotkey81.2 (uid 215,
# même coldkey) ; hotkey81 reste au mineur code.
MINER_HOTKEY=${RELIQUARY_HOTKEY:-hotkey81.2}
echo "[math] env=$RELIQUARY_ACTIVE_ENVS hotkey=$MINER_HOTKEY cap=$RELIQUARY_MAX_NEW_TOKENS dir=$MATH_DIR"
...
# Garde MATH (24/09) : la garde sha ci-dessus compare le TEMPLATE, pas le
# prompt réellement produit par l'env — elle n'avait pas vu que
# openmathinstruct.get_problem n'appelait pas le template (100 % de
# prompt_mismatch, disjoncteur sur la coldkey). On rend un prompt réel.
from reliquary.environment.openmathinstruct import OpenMathInstructEnvironment
_env = OpenMathInstructEnvironment()
_p = _env.get_problem(0)["prompt"]
assert c.PROTOCOL_VERSION >= 6, c.PROTOCOL_VERSION
assert _p.startswith("Solve the following math problem step by step.\n\n"), _p[:80]
assert _p.endswith("\n\nPut your final answer within \\boxed{}."), _p[-80:]
assert len(_env) == 13972791, len(_env)
print("[garde] prompt math rendu OK, univers", len(_env))
print("constantes OK:", c.PROTOCOL_VERSION, c.GENERATION_PROFILE_ID)
EOF

cd "$RELIQUARY_MINER_DIR"
exec /workspace/venv/bin/python -m reliquary.cli.main mine \
  --wallet-name camille81-v2 --hotkey "$MINER_HOTKEY" --network finney --netuid 81 \
