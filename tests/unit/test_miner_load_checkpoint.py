...
Updated for the v6/Qwen3.5 migration: the engine now loads via
``reliquary.shared.modeling.load_text_generation_model`` (which picks CausalLM
for legacy checkpoints and the conditional-generation wrapper for Qwen3.5)
instead of ``AutoModelForCausalLM.from_pretrained`` directly, and refreshes the
resolved EOS set after each swap. The behavioural contract is unchanged; only
the loader entry point and the EOS refresh are new.
"""
...
# Where the engine imported the loader (module-level import) — this is the
# name we patch, not the original in reliquary.shared.modeling.
_LOADER = "reliquary.miner.engine.load_text_generation_model"
...
def mock_engine():
    ...
    # tokenizer is consumed by _resolve_eos_ids (mock-like → ignored, so the
    # EOS set falls back to the hardcoded pair). Must exist as an attribute.
    eng.tokenizer = MagicMock(name="tok")
    eng.vllm_gpu = 0
    eng.proof_gpu = 1
    eng._eos_ids = [151643, 151645]
    return eng
...
def test_load_checkpoint_swaps_both_models(mock_engine):
    ...
    with patch(_LOADER, side_effect=[mock_hf, mock_gen]):
    ...
    # EOS set was refreshed against the new model (mock-like → hardcoded fallback).
    assert mock_engine._eos_ids == [151643, 151645]
...
def test_load_checkpoint_short_circuits_on_same_path(mock_engine):
    ...
    with patch(_LOADER, side_effect=[mock_hf, mock_gen]) as mock_loader:
        mock_engine._load_checkpoint("/tmp/checkpoint-5")
        mock_engine._load_checkpoint("/tmp/checkpoint-5")

    # loader should have been called exactly twice (once per model, first call only)
    assert mock_loader.call_count == 2


def test_load_checkpoint_hf_load_failure_keeps_old_models(mock_engine):
    """If the loader raises on hf load, old models stay."""
    original_hf = mock_engine.hf_model
    original_vllm = mock_engine.vllm_model

    with patch(_LOADER, side_effect=RuntimeError("HF load failed")):
...
def test_load_checkpoint_vllm_load_failure_sets_none(mock_engine):
    """If the second load (gen GPU) raises, hf is swapped but vllm is None."""
    mock_hf = _make_hf_mock("new_hf")

    with patch(_LOADER, side_effect=[mock_hf, RuntimeError("gen GPU OOM")]):
